...
export default {
  ...
  watch: {
    ...
    //     console.log('test', that.username, val)
    //     that.socket.emit('join', that.username, val);
    //   });
  ...
  mounted() {
    ...
    console.log(this.socket);
    this.socket.on('connect', function () {
      console.log('test', that.username, that.$route.query.id)
      that.socket.emit('join', that.username, that.$route.query.id);
    });
